...
let self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    methods: {
        ...
        getMenu() {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat-service/wechat/more/findMenus', { params: obj }).then(res => {
        ...
        delMenu(item) {
            this.$confirm(`删除后“${item.name}”菜单下设置的内容将被删除`, {
                ...
                if (item.parentzdyId) {
                    //删除子菜单
                    //拿到他的上级
                    let parent = this.customMenuLi.find(el => {
                        return el.zdyId === item.parentzdyId;
                    });
                    //  该子菜单在他父级的位置
                    let indx = parent.sub_button.findIndex(el => {
                        return item.zdyId === el.zdyId;
                    });
                    parent.sub_button.splice(indx, 1);
                } else {
                    // 删除菜单
                    let indx = this.customMenuLi.findIndex(el => {
                        return item.zdyId === el.zdyId;
                    });
            ...
            setTimeout(() => {
                self.$data.$validator.validateAll().then(pass => {
                    if (pass) {
                        ...
                        __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post('/api/wechat-service/wechat/more/createMenu', obj).then(res => {
                            if (res.code === '000') {
        ...
        hufu() {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get(`/api/wechat-service/wechat/more/recoveryMenu?gzhId=${this.gzh.appid}`).then(res => {
        ...
        delUp() {
            ...
            this.$confirm('此操作将删除线上菜单, 是否继续?', '提示', {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete(`/api/wechat-service/wechat/more/deleteMenu?gzhId=${self.gzh.appid}`).then(res => {
