...
new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    data: function data() {
        ...
        var mobileValidator = function mobileValidator(rule, value, callback) {
            ...
            _this.ruleForm.mobile = _this.ruleForm.mobile.replace(/\s/g, "");
            console.log(_this.ruleForm.mobile);
            if (_this.ruleForm.mobile.length < 1) {
                callback(new Error('请输入用户名'));
            } else if (m.test(_this.ruleForm.mobile) && _this.ruleForm.mobile.length === 11) {
                callback();
            } else {
    ...
    methods: {
        ...
        submitForm: function submitForm(formName) {
            ...
            this.$refs[formName].validate(function (valid) {
                if (valid) {
                    _this2.ruleForm.go = "登录中...";
                    console.log(_this2.ruleForm.checked);
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post('/login' + window.location.search, {
                        "mobile": _this2.ruleForm.mobile,
                        "password": _this2.ruleForm.password,
                        "keepMe": _this2.ruleForm.checked
                        ...
                        if (result.success) {
                            ...
                            _this2.ruleForm.ss = result.message;
                            _this2.ruleForm.go = "登录";
                        }
                    }).catch(function (error) {
                        _this2.ruleForm.ss = '网络连接错误，请稍后重试';
                        _this2.ruleForm.go = "登录";
                    });
                } else {
                    return false;
        ...
        //
        focusMobile: function focusMobile() {
            this.$refs.ruleForm.clearValidate();
        }
    },
    watch: {
        'ruleForm.password': function ruleFormPassword(val, oldVal) {
            if (val.length > 0) {
                this.ruleForm.ss = '';
            }
        }
