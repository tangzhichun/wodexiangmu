...
new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    methods: {
        ...
        getLeftList: function getLeftList() {
            ...
            var self = this;
            window.location.hash = self.$base64Encode(self.query);
            if (self.views.leftList) {
                return;
            }
            self.views.leftList = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat/admin/content/recommended/leftList').then(function (res) {
                self.views.leftList = false;
                if (res.data) {
                    _this.query.leftListTotalSize = res.data.size;
        ...
        getRightList: function getRightList(tou) {
            var _this2 = this;
            ...
            if (self.views.rightList) {
                return;
            }
            self.views.rightList = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat/admin/content/recommended/rightList', { params: obj }).then(function (res) {
                self.views.rightList = false;
                if (res.data) {
                    _this2.query.rightListTotalSize = res.data.totalSize;
                    _this2.rightList = res.data.results;
                }
            }).catch(function (err) {
                self.views.rightList = false;
            });
        ...
        //左右移动
        putLeftShift: function putLeftShift(e, type) {
            var _this3 = this;

            var obj = {
                id: e
            };
            if (this.views.guardRepetitive) {
                return;
            }
            this.views.guardRepetitive = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put('/api/wechat/admin/content/recommended/' + type, obj).then(function (res) {
                if (res.code === '000') {
                    _this3.views.guardRepetitive = false;
                    _this3.getLeftList();
                    _this3.getRightList();
                }
            }).catch(function (err) {
        ...
        //上下移动
        putUpAndDownShift: function putUpAndDownShift(e, i) {
            var _this4 = this;
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put('/api/wechat/admin/content/recommended/dragAndDropShift', obj).then(function (res) {
                _this4.views.guardRepetitive = false;
                if (res.code === '000') {
                    _this4.getLeftList();
                } else if (res.code === '815') {
                    _this4.$message('系统繁忙，请稍后重试');
                }
            }).catch(function (err) {
                _this4.views.guardRepetitive = false;
            });
        ...
        dragend: function dragend(ele, item) {
            ...
            var source = this.upAndDownShift.sortData;
            var target = this.upAndDownShift.targetData;
            if (source.id !== target.id) {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put('/api/wechat/admin/content/recommended/dragAndDropShift', obj).then(function (res) {
                    _this5.views.guardRepetitive = false;
                    _this5.upAndDownShift = { sortI: null, targetI: null, sortData: null, targetData: null };
                    if (res.code === '000') {
                        _this5.getLeftList();
                    } else if (res.code === '815') {
                        _this5.$message('系统繁忙，请稍后重试');
                    }
                }).catch(function (err) {
                    _this5.upAndDownShift = { sortI: null, targetI: null, sortData: null, targetData: null };
                    _this5.views.guardRepetitive = false;
