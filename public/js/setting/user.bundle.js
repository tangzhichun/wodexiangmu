...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__components_select_dsp_org_vue__ = __webpack_require__(173);
...
new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    delimiters: ['{[', ']}'],
    components: {
        'dsp-org': __WEBPACK_IMPORTED_MODULE_2__components_select_dsp_org_vue__["a" /* default */]
    },
    data: {
        meta: {
            EMPLOYEE_STATUS: __WEBPACK_IMPORTED_MODULE_3__scripts_module_enums__["default"].EMPLOYEE_STATUS,
            MENU_SCOPE: __WEBPACK_IMPORTED_MODULE_3__scripts_module_enums__["default"].MENU_SCOPE
        },
        // 界面输入的查询条件
        query: {
            mobile: null,
    ...
    methods: {
        ...
        fetchData: function fetchData() {
            ...
            var url = '/api/user/page/condition/' + self.query.currentPage + '/' + self.query.pageSize;
            var params = {
        ...
        disable: function disable(row) {
            var self = this;
            var url = '/api/user/status?id=' + row.id + '&status=' + !row.valid;
        ...
        reset: function reset(row) {
            var self = this;
            var url = '/api/user/resetPwd/' + row.id;
        ...
        //确认
        roleConfirm: function roleConfirm(ele) {
            var self = this;
            this.role.visibleBox = false;
            var params = this.role;
            var arr = params.value.join(',');
            ...
            Object(__WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"])({ method: 'post',
        ...
        //分配角色
        distributionRole: function distributionRole(row) {
            var self = this;
            this.role.id = row.id;
            this.role.visibleBox = true;
            var url = '/api/privilege/role/list/user/' + row.id;
            Object(__WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"])({ method: 'get', url: url }).then(function (response) {
                if (response.success) {
                    self.role.value = [];
        ...
        //查看所有职位
        clickPosition: function clickPosition(data) {
            var self = this;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/dealer/position/list/employee/' + data.id).then(function (response) {
                if (response.data) {
                    self.allPosition = response.data.filter(function (ele) {
                        if (!ele.primary) {
                            return ele;
                        }
                    });
        ...
        //查看权限
        lookPrivilege: function lookPrivilege(data) {
            var self = this;
            var reTree = JSON.parse(JSON.stringify(self.privilege.allPrivilege));
            this.view.privilege = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/privilege/auth/list/user/' + data.id).then(function (response) {
                if (response.data) {
                    var roleList = [];
                    var arr = JSON.parse(JSON.stringify(self.meta.MENU_SCOPE));
                    var arr2 = reTree.filter(function (pr) {
                        if (pr.children) {
                            pr.children = pr.children.filter(function (pr1) {
                                if (response.data[pr1.privilege]) {
                                    response.data[pr1.privilege].forEach(function (privilege) {
                                        if (!roleList.find(function (el) {
                                            return el.id == privilege.id;
                                        })) {
                        ...
                        if (response.data[pr.privilege]) {
                            response.data[pr.privilege].forEach(function (privilege) {
                                if (!roleList.find(function (el) {
                                    return el.id == privilege.id;
                                })) {
                                    roleList.push(privilege);
                    ...
                    arr.map(function (el) {
                        el.id = el.value;
                        el.name = el.label;
                        el.children = arr2.filter(function (ele) {
        ...
        //职位管理
        administrationPosition: function administrationPosition(type, data) {
            console.log(type);
            console.log(data);
            var query = {
                currentPage: 1,
            ...
            var sub = DSP.globalConfig.fileBaseUrl;
            var q = "";
    ...
    created: function created() {
        ...
        self.fetchData();
        var url = "/api/privilege/role/list?status=true";
        Object(__WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"])({
            method: 'get',
            url: url
        }).then(function (response) {
            ...
            return __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/menu/tree/parent');
        }).then(function (response) {
