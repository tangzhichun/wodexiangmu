...
var self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
        ...
        $validator: {
            rule: {
                information: {
                    ...
                    x_url: { required: true, customize: function customize($data, value) {
                            var pass = false;
                            if (value && value.substring(0, 8) === 'https://' || value.substring(0, 7) === 'http://') {
                                pass = true;
                            }
    ...
    methods: {
        ...
        addSub: function addSub(item) {
            ...
            if (item.sub_button.length < 1) {
                this.$confirm('添加子菜单后，一级菜单的内容将被清除。确定添加子菜单？', {
                    ...
                    var objs = Object.assign(item, _this.obj);
        ...
        getMenu: function getMenu() {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat-service/wechat/more/findMenus', { params: obj }).then(function (res) {
                _this2.views.getMenu = false;
                if (res.data && res.data.button) {
                    _this2.customMenuLi = res.data.button;
                    _this2.customMenuLi.map(function (el) {
                        if (!el.zdyId) {
                            el.zdyId = el.id;
                        }
                        if (el.sub_button && el.sub_button.length > 0) {
                            el.sub_button.map(function (sub) {
                                if (!sub.zdyId) {
                                    sub.zdyId = sub.id;
        ...
        delMenu: function delMenu(item) {
            ...
            this.$confirm('\u5220\u9664\u540E\u201C' + item.name + '\u201D\u83DC\u5355\u4E0B\u8BBE\u7F6E\u7684\u5185\u5BB9\u5C06\u88AB\u5220\u9664', {
                ...
                if (item.parentzdyId) {
                    //删除子菜单
                    //拿到他的上级
                    var parent = _this3.customMenuLi.find(function (el) {
                        return el.zdyId === item.parentzdyId;
                    });
                    //  该子菜单在他父级的位置
                    var indx = parent.sub_button.findIndex(function (el) {
                        return item.zdyId === el.zdyId;
                    });
                    parent.sub_button.splice(indx, 1);
                } else {
                    // 删除菜单
                    var _indx = _this3.customMenuLi.findIndex(function (el) {
                        return item.zdyId === el.zdyId;
                    });
                    _this3.customMenuLi.splice(_indx, 1);
                }
                _this3.information = null;
            });
        },
        save: function save() {
            var _this4 = this;
            ...
            var self = this;
            var flag = true;
            for (var k = 0; k < this.customMenuLi.length; k++) {
                ...
                if (this.customMenuLi[k].sub_button && this.customMenuLi[k].sub_button.length > 0) {
                    var arr = this.customMenuLi[k].sub_button;
                    var a = true;
            ...
            setTimeout(function () {
                self.$data.$validator.validateAll().then(function (pass) {
                    if (pass) {
                        var obj = {
                            button: _this4.customMenuLi,
                        ...
                        __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post('/api/wechat-service/wechat/more/createMenu', obj).then(function (res) {
                            if (res.code === '000') {
        ...
        //拖zhu
        dragstart: function dragstart(ele, item, indx) {
            this.upAndDownShift.sortData = JSON.parse(JSON.stringify(item));
            this.upAndDownShift.sortI = indx;
        },

        //放zhu
        dragend: function dragend(ele, item) {
            if (this.upAndDownShift.targetData && this.upAndDownShift.sortData && this.upAndDownShift.sortI !== this.upAndDownShift.targetI) {
                var sI = this.upAndDownShift.sortI;
                var so = this.upAndDownShift.sortData;
                var to = this.upAndDownShift.targetData;
                var tI = this.upAndDownShift.targetI;
        ...
        //移动到item主
        dragenter: function dragenter(ele, item, i) {
        ...
        //拖zhu
        dragstartSub: function dragstartSub(ele, indx, arr) {
            this.upAndDown.sD = ele;
            this.upAndDown.sI = indx;
        },

        //放zhu
        dragendSub: function dragendSub(ele, item, fI) {
            var sI = this.upAndDown.sI;
            var sD = this.upAndDown.sD;
            var tD = this.upAndDown.tD;
            var tI = this.upAndDown.tI;
            var arr = this.customMenuLi[fI] && this.customMenuLi[fI].sub_button;
            if (sD && tD && arr && tI !== sI) {
                arr[sI] = tD;
                arr[tI] = sD;
        ...
        //移动到item主
        dragenterSub: function dragenterSub(item, i, arr) {
        ...
        hufu: function hufu() {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat-service/wechat/more/recoveryMenu?gzhId=' + this.gzh.appid).then(function (res) {
                _this5.views.hufu = false;
                if (res.code === '000') {
                    _this5.getMenu();
                }
            }).catch(function (err) {
        ...
        delUp: function delUp() {
            ...
            this.$confirm('此操作将删除线上菜单, 是否继续?', '提示', {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete('/api/wechat-service/wechat/more/deleteMenu?gzhId=' + self.gzh.appid).then(function (res) {
