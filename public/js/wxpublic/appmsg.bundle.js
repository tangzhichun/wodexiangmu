...
var self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    methods: {
        ...
        fetchData: function fetchData(e) {
            ...
            var self = this;
            if (self.views.listView) {
                return;
            }
            var tab = this.query.gzh;
            var obj = {
                gzhId: tab.appid
            };
            window.location.hash = self.$base64Encode(self.query);
            self.views.listView = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat-service/wechat/more/queryNewArticleList', { params: obj }).then(function (res) {
                self.views.listView = false;
                if (res.data) {
                    _this.tab0Data = res.data;
                } else {
                    _this.tab0Data = [];
                }
        ...
        //新建或者编辑图片素材
        appOrEditmsg: function appOrEditmsg(e, msgid) {
            var obj = {
        ...
        getOnLine: function getOnLine(e) {
            if (self.views.listView) {
                return;
            }
            self.views.listView = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat-service/wechat/more/pullReplyContentArticles?gzhId=' + e.appid).then(function (res) {
                self.views.listView = false;
                if (res.code === '000') {
                    self.fetchData();
                }
        ...
        delMsg: function delMsg(item) {
            ...
            this.$confirm('此操作将永久删除该图文, 是否继续?', '提示', {
                ...
                if (!self.views.delLoading) {
                    ...
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete('/api/wechat-service/wechat/more/delNewsMaterial?newsId=' + item.id + '&configId=' + item.configId + '&gzhId=' + _this2.query.gzh.appid).then(function (res) {
                        loading.close();
    ...
    created: function created() {
        ...
        window.onresize = function () {
            var dom = document.getElementsByClassName('msg-box')[0];
