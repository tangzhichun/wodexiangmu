...
var self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    data: {
        ...
        tab: [{ label: '单图模版', value: 'one' }, { label: '多图模版', value: 'more' }],
        query: {
            // 总条数
            totalSize: 0,
            // 每页多少条
    ...
    methods: {
        indexMethod: function indexMethod(index) {
            var i = this.designQuery.size * (this.designQuery.current - 1) + 1;
            return i + index;
        },
        ...
        addTemplate: function addTemplate(e, type) {
            var url = '/cms/editTemplate?templateType=' + this.query.templateType + '&type=' + type;
            if (typeof e === 'string') {
        ...
        getTemplate: function getTemplate() {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat/admin/share/template/' + self.query.currentPage + '/' + self.query.pageSize, { params: params }).then(function (res) {
                _this.dataList = res.data && res.data.results;
            });
        },
        deleteInformation: function deleteInformation(e) {
            this.$confirm('此操作将永久删除该模版, 是否继续?', '提示', {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete('/api/wechat/admin/share/template/' + e.id).then(function (res) {
                    if (res.code === '000') {
                        self.getTemplate();
                        self.$message({
                            type: 'success',
        ...
        //打开拥有此模版的方案列表
        openDesignList: function openDesignList(e) {
            this.designQuery.template = e;
            this.views.designList = true;
            this.queryDesignList();
        },
        queryDesignList: function queryDesignList() {
            var _this2 = this;

            var self = this;
            this.views.designListLoading = true;
            this.designQuery.current = 1;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat/admin/design/' + self.designQuery.template.id + '/template').then(function (res) {
                _this2.views.designListLoading = false;
                if (res.code === '000') {
        ...
        location: function location(type, e) {
            var hashObj = {
                leftData: {
                    //具体选中了那个
                    column: {
        ...
        queryDesignTitle: function queryDesignTitle() {
            var self = this;
            this.designQuery.current = 1;
            var arr = self.designList.filter(function (el) {
                return el.title.indexOf(self.designQuery.designTitle) > -1;
            });
