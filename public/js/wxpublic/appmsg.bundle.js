...
let self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    methods: {
        ...
        fetchData(e) {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/wechat-service/wechat/more/queryNewArticleList', { params: obj }).then(res => {
        ...
        getOnLine(e) {
            ...
            self.views.listView = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get(`/api/wechat-service/wechat/more/pullReplyContentArticles?gzhId=${e.appid}`).then(res => {
                self.views.listView = false;
                if (res.code === '000') {
                    self.fetchData();
                }
        ...
        delMsg(item) {
            ...
            this.$confirm('此操作将永久删除该图文, 是否继续?', '提示', {
                ...
                if (!self.views.delLoading) {
                    ...
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete(`/api/wechat-service/wechat/more/delNewsMaterial?newsId=${item.id}&configId=${item.configId}&gzhId=${this.query.gzh.appid}`).then(res => {
                        loading.close();
