    ...
    methods: {
        ...
        tagDeleta(el) {
            this.$confirm(`此操作将永久删除${el.alias}标签, 是否继续?`, '提示', {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete(`/api/tag/delete/id/${el.id}`).then(res => {
                    if (res.success) {
                        diction.fetchData();
                        diction.$message({
                            type: 'success',
        ...
        saveAddTag(type) {
            ...
            this.$data.$validator.validateAll().then(pass => {
                if (pass) {
                    ...
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post(`/api/tag/save?realm=${diction.addTagParams.realm}&name=${diction.addTagParams.name}&alias=${diction.addTagParams.alias}&interior=true`).then(res => {
                        if (res.success) {
                            diction.view = 1;
                            diction.$message({
                                type: 'success',
        ...
        look(el, event, column) {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get(`/api/tag/entity/list/tag?tagId=${el.id}&realm=${el.realm}`).then(res => {
                if (res.success) {
        ...
        //编辑，保存标签
        tagEditSave() {
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put(`/api/tag/update?realm=${diction.editTagParams.realm}&name=${diction.editTagParams.name}&alias=${diction.editTagParams.alias}&interior=${diction.editTagParams.interior}&id=${diction.editTagParams.id}`).then(res => {
                if (res.success) {
        ...
        unbind(ele) {
            this.$confirm(`此操作将给${ele.key1}解绑${diction.currentTag.tag.alias}标签, 是否继续?`, '提示', {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post(`/api/content/unbind?realm=${diction.currentTag.tag.realm}&entityId=${ele.ENTITY_ID}&tagId=${diction.currentTag.tag.id}&isManager=true`).then(res => {
                    if (res.success) {
