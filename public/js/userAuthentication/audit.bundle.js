...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__components_ui_dsp_img_vue__ = __webpack_require__(66);
...
new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    delimiters: ['{[', ']}'],
    components: {
        'dsp-img': __WEBPACK_IMPORTED_MODULE_2__components_ui_dsp_img_vue__["a" /* default */]
    },
    data: {
        meta: {
            position: [],
            CXHHR_REJECT_REASON: __WEBPACK_IMPORTED_MODULE_3__scripts_module_enums__["default"].dict('CXHHR_REJECT_REASON')
        },
        tab: [{ label: '待审核', value: 'tab0' }, { label: '已通过', value: 'tab1' }, { label: '已拒绝', value: 'tab2' }],
    ...
    methods: {
        changeTab: function changeTab(ba) {
        ...
        fetchData: function fetchData(page) {
            var self = this;
            var query = self.query[self.query.currTab];
            //构造地址栏hash参数
            window.location.hash = self.$base64Encode(self.query);
            var url = '/api/mpuser/service/promotion/list';
            var params = {
        ...
        //打开对话框
        openAudit: function openAudit(el) {
            var self = this;
            ...
            self.views.loading = true;
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/mpuser/service/promotion/selectRemark?id=' + el.id).then(function (res) {
                self.views.loading = false;
                if (res.code === '000') {
        ...
        save: function save(type) {
            ...
            function f(obj) {
                if (self.views.auditL) {
                    return;
                }
                self.views.auditL = true;
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put('/api/mpuser/service/promotion/updateStatus', obj).then(function (res) {
        ...
        //关闭对话框
        closeAudit: function closeAudit() {
    ...
    created: function created() {
        var self = this;
        var arr = JSON.parse(JSON.stringify(__WEBPACK_IMPORTED_MODULE_3__scripts_module_enums__["default"].dict('PARTNERSHIP')));
        arr.map(function (el) {
