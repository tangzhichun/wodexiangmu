...
let bindFieldValidator = function (el, binding, vnode) {
    ...
    // 解析校验规则
    let rules = util.getProp(vnode.context, '$data.$validator.rule');
    let rule = util.getProp(rules, ruleProp);
    if(!rule) {
        //没有规则，不做校验
    ...
    let fieldValidator = function (value) {
        let val = value || util.getProp(vnode.context, '$data.' + prop);
...
export default {
            ...
            if (prop) {
                vnode.context.$watch(prop, function (newVal, oldVal) {
                    ...
                    if (newVal && !(regExp.test(newVal))) {
                        ...
                        util.setProp(vnode.context, '$data.' + prop, oldVal);
                    }
                });
            } else {
    ...
    trim: {
        bind: function (el, binding, vnode) {
            ...
            if (prop) {
                vnode.context.$watch(prop, function (newVal, oldVal) {
                    util.setProp(vnode.context, '$data.' + prop, newVal && newVal.trim() || oldVal);
                });
            } else {
