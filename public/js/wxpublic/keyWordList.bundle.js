...
new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    methods: {
        ...
        fetchData: function fetchData(e) {
            var self = this;
            var tab = this.axiosQuery.gzh;
            var query = this.axiosQuery[tab.query];
            if (e === 1) {
                query.currentPage = 1;
            }
            //构造地址栏hash参数
            window.location.hash = self.$base64Encode(self.axiosQuery);
            var url = '/api/wechat-service/wechat/more/findNewsKeywordsList';
            var obj = {
        ...
        //新增修改关键词回复
        addKeywordView: function addKeywordView(e) {
            if (e === 'add') {
    ...
    created: function created() {
        ...
        this.tab = DSP.gzh.map(function (el, i) {
            el.query = 'query' + i;
            //查询关键词查询条件
