...
var self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    data: {
        ...
        meta: {
            positionS: __WEBPACK_IMPORTED_MODULE_2__scripts_module_enums__["default"].dict('PARTNERSHIP'),
            dealerEnums: __WEBPACK_IMPORTED_MODULE_2__scripts_module_enums__["default"].dict('CMS_DEALER_ENUMERATE'),
            position: []
        },
        tab: [{ label: '员工', value: 'tab0' }, { label: '合作方', value: 'tab2' }, { label: '其他', value: 'tab4' }],
        //上传对象
        importObj: {
    ...
    methods: {
        indexMethod: function indexMethod(index, tab) {
            var i = this.query[tab + 'Query'].pageSize * (this.query[tab + 'Query'].currentPage - 1) + 1;
            return i + index;
        },
        ...
        changeAgent: function changeAgent(e) {
            this.query.agent = e;
        ...
        fetchData: function fetchData(e) {
            ...
            var self = this;
            var url = '/api/mpuser/service/agent/list';
            var tab = this.query.currTab;
            if (!tab) {
                this.$message.error('请先选择tab');
                return;
            }
            var query = this.query[tab + 'Query'];
            if (e === 1) {
                query.currentPage = 1;
            }
            console.log(query);
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get(url, { params: obj }).then(function (res) {
                self.views.loading = false;
                if (res.data) {
                    _this[tab + 'Data'] = res.data.results;
        ...
        openAddDialog: function openAddDialog(e, obj) {
            if (obj) {
                ...
                _obj.category = e.charAt(e.length - 1);
                this.information = Object.assign({}, _obj);
            }
        ...
        save: function save() {
            ...
            this.$data.$validator.validateAll().then(function (pass) {
                if (pass) {
                    var method = self.information.id ? 'put' : 'post';
                    var url = self.information.id ? '/api/mpuser/service/agent/update' : '/api/mpuser/service/agent/add';
                    var obj = {
        ...
        delAgent: function delAgent(e) {
            ...
            this.$confirm(title, '提示', {
                ...
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete('/api/mpuser/service/agent/delete', { params: obj }).then(function (res) {
                    if (res.code !== '000') {
        ...
        //上传之前判断文件大小和格式
        importBeforeUpload: function importBeforeUpload(file) {
            var self = this;
        ...
        //上传成功后
        importUploadSucceed: function importUploadSucceed(response, file, fileList) {
            var self = this;
            ...
            if (response.success && response.code === '000') {
                ...
                switch (response.code) {
                    case '804':
                        title = '手机号重复';break;
                    case '851':
                        title = '请检查文件，除备注都是必填字段';break;
                    case '850':
                        title = '请检查文件，所填写字段长度太长';break;
                    case '70011':
                        title = '无效excel';break;
                    case '70012':
                        title = '导入excel为空';break;
                    case '70013':
                        title = '导入excel失败';break;
                    case '70014':
                        title = '导入excel失败';break;
                    case '70015':
                        title = 'excel数据太多';break;
                    default:
                        title = '当前网络不佳，请稍后重试';
                }
                self.$message({
        ...
        //上传失败后
        importUploadFailed: function importUploadFailed(response, file, fileList) {
            var self = this;
            self.importObj.isUploading = false;
    ...
    created: function created() {
        var self = this;
        var arr = JSON.parse(JSON.stringify(DSP.PARTNERSHIP));
        arr.map(function (el) {
