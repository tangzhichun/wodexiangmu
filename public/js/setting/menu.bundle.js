    ...
    methods: {
        ...
        saveEditMenu(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    var url = '/api/menu';
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put(url, this.editMenuForm).then(response => {
                        if (response.success) {
                            this.mode = 'view';
                            this.$message.success('保存成功！');
                            this.getMenus();
        ...
        saveEditPrivilege() {
            ...
            self.$refs[formName].validate(valid => {
                if (valid) {
                    var url = '/api/menu';
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put(url, self.button).then(response => {
                        if (response.success) {
                            self.mode = 'view';
                            self.$message({
                                message: '修改成功',
                            ...
                            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get(url2).then(response => {
        ...
        saveEditSubMenu(formName) {
            ...
            self.$refs[formName].validate(valid => {
                if (valid) {
                    var url = '/api/menu';
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put(url, self.newMenu).then(response => {
                        if (response.success) {
                            self.mode = 'view';
        ...
        save(val) {
            var self = this;
            var url = "/api/menu";
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post(url, val).then(response => {
                if (response.success) {
