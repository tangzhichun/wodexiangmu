...
var roleSetting = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    el: '#app',
    delimiters: ['{[', ']}'],
    data: {
    ...
    methods: {
        //排序
        objectArraySort: function objectArraySort(keyName) {
        ...
        changeList: function changeList(item, index) {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/privilege/role/' + item.id).then(function (response) {
                var result = response;
                if (result.success) {
        ...
        submitForm: function submitForm(formName) {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post(url, roleSetting.newRole).then(function (response) {
                if (response.success) {
        ...
        editForm: function editForm(formName) {
            ...
            var url = '/api/privilege/role';
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put(url, roleSetting.newRole).then(function (response) {
                if (response.success) {
        ...
        delRole: function delRole(item) {
            var url = '/api/dealer-service/privilege/role/' + item.id;
            var h = this.$createElement;
            this.$msgbox({
                title: '提示',
                message: h('p', null, [h('span', null, '此操作将永久删除'), h('span', { style: 'color: red' }, item.name), h('span', null, '角色, 是否继续? ')]),
        ...
        //    在树形结构上再加一级
        addTree: function addTree(oldArr) {
            var arr1 = JSON.parse(JSON.stringify(this.MENU_SCOPE));
            var a = JSON.parse(JSON.stringify(oldArr));
            var arr = arr1.map(function (el) {
                el.id = el.value;
                el.name = el.label;
                el.children = a.filter(function (ele) {
    ...
    created: function created() {
        ...
        var obj = self.$base64Decode(hash);
        setTimeout(function () {
            if (hash != '') {
                console.log(obj);
        ...
        __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get(url).then(function (response) {
            ...
            if (result.success) {
                // 储存所有的权限信息
                roleSetting.menuTrees = _this.addTree(result.data);
                ...
                // 获取第一个的权限
                if (!obj || !obj.item) {
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/privilege/role/' + roleSetting.roleList[0].id).then(function (response) {
                        var result = response;
                        if (result.success) {
                            roleSetting.newRole = JSON.parse(JSON.stringify(result.data));
    ...
    computed: {
        // 自定义筛选器
        rolesFilter: function rolesFilter() {
            var self = this;
