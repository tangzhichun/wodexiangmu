...
var diction = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    el: '#app',
    delimiters: ['{[', ']}'],
    data: {
        ...
        meta: {
            TAGSCOPE: __WEBPACK_IMPORTED_MODULE_2__scripts_module_enums__["default"].TAGSCOPE,
            DELIVERY_PART: __WEBPACK_IMPORTED_MODULE_2__scripts_module_enums__["default"].DELIVERY_PART,
            BRAND: __WEBPACK_IMPORTED_MODULE_2__scripts_module_enums__["default"].BRAND
        },
        // 界面输入的查询条件
        query: {
            alias: null, //名称     模糊检索
    ...
    methods: {
        ...
        fetchData: function fetchData() {
            ...
            var url = '/api/tag/list/condition';
            self.loading = true;
            var params = {
        ...
        //删除标签
        tagDeleta: function tagDeleta(el) {
            this.$confirm('\u6B64\u64CD\u4F5C\u5C06\u6C38\u4E45\u5220\u9664' + el.alias + '\u6807\u7B7E, \u662F\u5426\u7EE7\u7EED?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(function () {
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].delete('/api/tag/delete/id/' + el.id).then(function (res) {
                    if (res.success) {
                        diction.fetchData();
                        diction.$message({
                            type: 'success',
        ...
        //新增内置标签
        addTag: function addTag(el) {
            this.view = 2;
        },

        //保存 || 取消 新标签
        saveAddTag: function saveAddTag(type) {
            if (type === 'quxiao') {
            ...
            this.$data.$validator.validateAll().then(function (pass) {
                if (pass) {
                    ...
                    __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post('/api/tag/save?realm=' + diction.addTagParams.realm + '&name=' + diction.addTagParams.name + '&alias=' + diction.addTagParams.alias + '&interior=true').then(function (res) {
                        if (res.success) {
                            diction.view = 1;
                            diction.$message({
                                type: 'success',
        ...
        look: function look(el, event, column) {
            this.currentTag.tag = el;
            this.$refs.singleTable.setCurrentRow(el);
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].get('/api/tag/entity/list/tag?tagId=' + el.id + '&realm=' + el.realm).then(function (res) {
                if (res.success) {
        ...
        //转跳页面
        changeHref: function changeHref(el) {
            var type = this.currentTag.tag.realm;
            var url = '';
            ...
            if (type === 'POSITION') {
                ...
                var a = diction.$base64Encode(obj);
        ...
        //点击编辑标签
        tagEdit: function tagEdit(el) {
            this.editTagParams = JSON.parse(JSON.stringify(el));
        },

        //编辑，保存标签
        tagEditSave: function tagEditSave() {
            var _this = this;

            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put('/api/tag/update?realm=' + diction.editTagParams.realm + '&name=' + diction.editTagParams.name + '&alias=' + diction.editTagParams.alias + '&interior=' + diction.editTagParams.interior + '&id=' + diction.editTagParams.id).then(function (res) {
                if (res.success) {
                    _this.editTagParams = { id: null, alias: null };
                    diction.$message({
                        type: 'success',
                        message: '修改成功'
                    });
        ...
        //    为某个实体解绑
        unbind: function unbind(ele) {
            this.$confirm('\u6B64\u64CD\u4F5C\u5C06\u7ED9' + ele.key1 + '\u89E3\u7ED1' + diction.currentTag.tag.alias + '\u6807\u7B7E, \u662F\u5426\u7EE7\u7EED?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(function () {
                __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post('/api/content/unbind?realm=' + diction.currentTag.tag.realm + '&entityId=' + ele.ENTITY_ID + '&tagId=' + diction.currentTag.tag.id + '&isManager=true').then(function (res) {
                    if (res.success) {
