...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__components_ui_xcx_page_skip_vue__ = __webpack_require__(162);
/* jshint esversion: 6 */
...
var self = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    ...
    components: {
        ...
        'xcx-page-skip': __WEBPACK_IMPORTED_MODULE_4__components_ui_xcx_page_skip_vue__["a" /* default */]
    },
    data: {
        //行政区域业务字典
        adRigion: __WEBPACK_IMPORTED_MODULE_2__scripts_module_enums__["default"].dict('ADMINISTRATIVE_REGION'),
        // 页面类型
    ...
    methods: {
        ...
        // 获得指定group的广告
        getBannerDetail: function getBannerDetail(ban, i, n) {
            var that = this;
        ...
        editBanner: function editBanner(banner, i) {
            var that = this;
            // 当前广告位的数据
            ...
            if (that.currenItem && that.currenItem.onclickUrl && that.currenItem.onclickUrl === '/pages/design/listBytag/main') {
                var arr = [];
                var el = DSP.tags.tags.find(function (ele) {
                    return ele.code === that.currenItem.onclickParams;
                });
                arr[0] = el && el.type;
        ...
        // 删除指定广告位的某个图片
        deleteIndexImage: function deleteIndexImage(item, index) {
            var that = this;
        ...
        saveAndSubmit: function saveAndSubmit() {
            ...
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].post(url, item).then(function (response) {
        ...
        // 修改并保存按钮点击
        editAndSubmit: function editAndSubmit() {
            var that = this;
            ...
            console.log(item);
            // if (1===1){return;}
            __WEBPACK_IMPORTED_MODULE_1__scripts_module_axios__["default"].put(url, item).then(function (response) {
        ...
        // 图片更改的整理
        bannerChange: function bannerChange(val) {
            var that = this;
            that.currenItem.onclickParams = null;
            that.tagPram = null;
        },
        onclickParams: function onclickParams() {
            var that = this;
        ...
        // 更改排序
        changeOrder1: function changeOrder1(direct, item, index) {
            var that = this;
            var url = '/api/wechat/admin/banner/update/id';
            // var index
            if (direct === 'right') {
                ...
                var _currenItem = JSON.parse(JSON.stringify(that.currenItem));
                that.currentBanner.groupDetails.splice(index - 1, 0, _currenItem);
            }
            that.editAndSubmit();
        },
        changeOrder: function changeOrder(direct, item, index) {
            var that = this;
            var arr = [];
