...
var message = new __WEBPACK_IMPORTED_MODULE_0__scripts_module_vue__["default"]({
    el: '#app',
    delimiters: ['{[', ']}'],
    data: {
        loading: false,
    ...
    methods: {
        ...
        indexMethod: function indexMethod(index) {
            var i = this.pagination.pageSize * (this.pagination.currentPage - 1) + 1;
            return i + index;
        },

        //把对象变成URL查询字符串
        urlCord: function urlCord(url, data) {
            ...
            var flag = false; //标识query属性是否为空
            var param = ""; //带问号的查询参数字符串
            for (var k in data) {
                //当前查询值不为空时则不拼接
                if (data[k]) {
        ...
        getUrl: function getUrl() {
            ...
            var url = this.urlCord("/api/message/list/" + this.pagination.currentPage + "/" + this.pagination.pageSize, obj);
            var search = this.urlCord("?page=" + this.pagination.currentPage + "&size=" + this.pagination.pageSize + '&totalSize=' + this.pagination.totalSize + '&', obj);
        ...
        doQuery: function doQuery() {
            ...
            var self = this;
            this.loading = true;
            var flag = this.validate.mobile(this.query.receiver);
            if (flag) return;
    ...
    created: function created() {
        var urlQuery = this.getQueryObject();
        if (urlQuery.page === 'undefined') {
            this.pagination.currentPage = 1;
        } else {
            this.pagination.currentPage = Number(urlQuery.page) || 1;
        }
        ...
        if (urlQuery.receiver) {
            this.query.receiver = urlQuery.receiver;
        }
        if (urlQuery.type) {
            this.query.type = urlQuery.type;
        }
        this.doQuery();
    },
    watch: {
        //处理返回值的登录结果
        tableData: function tableData(to, from) {
            for (var obj in to) {
                var flag = to[obj].success;
                if (flag) {
                    to[obj].success = "成功";
                } else {
                    to[obj].success = "失败";
